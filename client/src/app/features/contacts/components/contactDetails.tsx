...
const ContactDetails: React.FC<ContactDetailsProps> = ({ contactId }) => {
    ...
    return (
        <div className=" relative w-[920px] h-[706px] bg-[#F8F8F8] rounded-[30px] mx-auto mt-[88px] flex flex-col items-center">

            <button 
                onClick={handleEdit} 
                className="absolute top-4 right-4 w-6 h-6 flex items-center justify-center text-[#111928] mt-[26px] mr-[36px]">
            ...
            <div className="w-[194px] h-[194px] rounded-full border-[3px] border-[#9378FF] flex items-center justify-center mt-[61px]">
                <Image 
                    src={contact.profilePicture} 
                    alt={`${contact.name} profile`}
                    width={178}
                    height={178}
                    className="rounded-full object-cover w-[178px] h-[178px]"
                />
            </div>
            
            <h2 className="font-redhat text-[30px] font-bold leading-[39.69px] text-black mt-[38px]">
                {contact.name}
            </h2>
                <h2 className="font-redhat text-[20px] font-bold leading-[26.46px] w-[79px] h-[26px] mt-[38px]">
                    Address
                </h2>
                <p
                className="font-public-sans max-w-[36] text-[16px] font-normal leading-[25.6px] text-[#99879D] mt-[13px]">
                    {contact.address}
                </p>
                <h2 className="font-redhat text-[20px] font-bold leading-[26.46px] w-[79px] h-[26px] mt-[13px]">
                    Phone
                </h2>
                <p
                className="font-public-sans max-w-[36] text-[16px] font-normal leading-[25.6px] text-[#99879D] mt-[6px]">
                    {contact.phoneNumber}
                </p>
                <h2 className="font-redhat text-[20px] font-bold leading-[26.46px] w-[79px] h-[26px] mt-[13px]">
                    Email
